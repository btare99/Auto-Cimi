...
// Use Windows system certificates (fixes SSL issues without hacks)
if (process.platform === 'win32') {
  try {
    require('win-ca');
    console.log('🛡️ Sistemi i certifikatave të Windows u ngarkua.');
  } catch (err) {
    console.warn('⚠️ win-ca nuk u ngarkua (nuk është e nevojshme në Linux).');
  }
}
...
app.listen(PORT, () => {
  ...
  console.log('📌 Rrugët e regjistruara: /api/parts, /api/orders, /api/coming-cars, /api/reservations');
});
